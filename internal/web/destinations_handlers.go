...
func (s *Server) handleDestinationsPlexPinStart(c *gin.Context) {
	...
	var sb strings.Builder
	sb.WriteString(`<div data-plex-pin-waiting class="info-box" style="border-color:var(--info);margin:.5rem 0" role="status" aria-live="polite">`)
	...
	sb.WriteString(`" target="plexAuth" rel="noreferrer">open plex.tv sign-in</a>.`)
	sb.WriteString(`</div>`)
	sb.WriteString(`<script>(function(){`)
	sb.WriteString(`try{window.__audplexusPlexAuthPopup=window.open(`)
	sb.WriteString(authURLJS)
	sb.WriteString(`,"plexAuth","width=540,height=760,resizable=yes,scrollbars=yes");}catch(e){}`)
...
func (s *Server) handleDestinationsPlexPinPoll(c *gin.Context) {
	...
	sb.WriteString(`var s=document.getElementById('plex-signin-btn');if(s){s.disabled=true;s.setAttribute('aria-disabled','true');s.textContent='Signed in with Plex';}`)
	sb.WriteString(`var pr=document.getElementById('plex-pin-result');if(pr){var wmsg=pr.querySelector('[data-plex-pin-waiting]');if(wmsg){wmsg.remove();}}`)
	sb.WriteString(`try{var w=window.__audplexusPlexAuthPopup;if(w&&!w.closed){w.close();}else{var n=window.open('','plexAuth');if(n&&!n.closed){n.close();}}}catch(e){}`)
...
func (s *Server) handleDestinationsPlexDiscoverServers(c *gin.Context) {
	...
	sb.WriteString(`var u=document.getElementById('url');if(u){u.value=this.value;u.dispatchEvent(new Event('change',{bubbles:true}));}`)
	sb.WriteString(`var sr=document.getElementById('plex-section-discover-result');if(sr){sr.innerHTML='';}`)
...
func (s *Server) handleDestinationsPlexDiscoverSections(c *gin.Context) {
	...
	sb.WriteString(`onchange="var s=document.getElementById('plex_section_id');if(s){s.value=this.value;s.dispatchEvent(new Event('change',{bubbles:true}));}">`)
